...
transporter.prototype.get = function(src, done){
  ...
  var from_cache = function(src){
    ...
    self.cache.get(utils.getFilenameBySrc(src, self.opts.setFormat), function(err, cached_file){
      if (!err)
  ...
  var from_store = function(src){
    if (!self.opts.store)
     ...
     self.store.get(utils.getFilenameBySrc(src, self.opts.setFormat), function(err, stored_file){
        if (!err)
