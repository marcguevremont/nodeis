...
processer.prototype.store = function(img, done){
   ...
   self.transporter.store.set(utils.getFilenameBySrc(self.req.query['src'], config.options.setFormat), p, function(err){
        if (err)  
          return done(err);
...
processer.prototype.cache = function(img, done){
  ...
  self.transporter.cache.set(utils.getFilenameBySrc(self.req.query['src'], config.options.setFormat),p , function(err, cache_file){
         if (err)  
          return done(err);
