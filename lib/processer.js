...
function processer(req, res, isdone) {
    ...
    if (method == 'fetch' || method == "webshot" || (method == 'image' && req.method == "GET")){
      ...
      self.transporter.get(query['src'], function(err, img, from){
        ...
        if (!err)
        {
            ...
            else if (from == 'url' || from == 'webshot') 
            {
              ...
              //Default parameter to save the file before displaying
              var d_opts = {};
              
              //If it is a webshot we use different setting
              if (from == 'webshot'){
                if (query.viewport){
                  var ww = config.options.webshot.viewport.lg.width;
                  
                  if (query.viewport == "sm")
                    ww = config.options.webshot.viewport.sm.width; 
                  else if (query.viewport == "md")
                    ww = config.options.webshot.viewport.md.width;  
                    
                  d_opts = {'query': {'opt':{'cropTo':[ww,config.options.height]}}}
                }
              }
              
              self.render(img, d_opts, function(err, image){
        ...
        else
        {
         ...
         else if (form.type == null){ //Utility method
...
processer.prototype.store = function(img, done){
   ...
   self.transporter.store.set(utils.getFilenameByQuery(self.req.query, config.options.setFormat), p, function(err){
        if (err)  
          return done(err);
...
processer.prototype.cache = function(img, done){
  ...
  self.transporter.cache.set(utils.getFilenameByQuery(self.req.query, config.options.setFormat),p , function(err, cache_file){
         if (err)  
          return done(err);
