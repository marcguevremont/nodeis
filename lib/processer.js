...
function processer(req, res, isdone) {
    ...
    // if (!(this instanceof processer)){
     // return new processer(req, res, done);
    // }
...
processer.prototype.store = function(img, done){
   ...
   self.transporter.store.set(utils.getFilenameBySrc(self.req.query['src']), p, function(err){
        if (err)  
          return done(err);
...
processer.prototype.cache = function(img, done){
  ...
  self.transporter.cache.set(utils.getFilenameBySrc(self.req.query['src']),p , function(err, cache_file){
         if (err)  
          return done(err);
